...
is_catch <- 
    filter(is_catch, functional_group == 'FCD') %>%
...
landings.plot <- 
    ...
    geom_line(data=bmt.landings, aes(x=year, y=landings), color='black') + 
...
spr.ldist.plot <-
    ggplot(data=spr.ldist, aes(x=length, y=number)) + geom_line(size=1.5) +
...
spr.aldist.plot <-
    ggplot(data=spr.aldist, aes(x=age, y=count)) + geom_line(size=1.5) +
